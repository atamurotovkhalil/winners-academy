...
const FreeSharing = () => {
  ...
  }, [articles.length, getArticles]); // Re-run only if articles length changes

  const memoizedArticles = useMemo(() => articles, [articles]); // Memoize articles to prevent unnecessary re-renders
  const freearticles = memoizedArticles.filter((art)=>art.category ==="freesharing")
  return (
